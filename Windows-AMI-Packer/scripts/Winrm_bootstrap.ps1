...
# AWS - To enable user data execution on Windows Server 2019 or later (EC2Launch)
...
mkdir -p c:\ssh
$openSSHZip = Join-Path 'C:\ssh\' 'OpenSSH.zip'
...
Move-Item -Path (Join-Path 'C:\ssh\' 'OpenSSH-Win64') `
...
icacls 'C:\ProgramData\ssh\authorized_keys' /inheritance:r
icacls 'C:\ProgramData\ssh\authorized_keys' /grant SYSTEM:`(F`)
icacls 'C:\ProgramData\ssh\authorized_keys' /grant BUILTIN\Administrators:`(F`)
icacls 'C:\ProgramData\ssh\sshd_config' /inheritance:r
icacls 'C:\ProgramData\ssh\sshd_config' /grant SYSTEM:`(F`)
...
icacls 'C:\ProgramData\ssh\' /grant BUILTIN\Administrators:`(F`)
Copy-Item 'C:\ProgramData\ssh\authorized_keys' 'C:\Program Files\OpenSSH\'
Copy-Item 'C:\ProgramData\ssh\sshd_config' 'C:\Program Files\OpenSSH\'
cd 'C:\Program Files\OpenSSH\'
cmd.exe /c 'C:\Program Files\OpenSSH\ssh-keygen.exe' -A
